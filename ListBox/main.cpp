...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	//hPrevInst - предыдущий запущенный экземпляр нашей проги. Этот параметр использовался только в win98.
	//lpCmdLine - командная строка, с кот. б/запущена прога
	//nCmdShow - ружим отображения окна. Н-р, развернуто на весь экран, свернуто в окно, сренуто на панель задач...
...
BOOL CALLBACK DlgProcAdd(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			...
			{
				MessageBox(hwnd, "Такое знаычение уже есть в списке, введите другое значение", "Опаньки", MB_OK | MB_ICONINFORMATION);
			}
				break;
		}
			//break;
